document.addEventListener('DOMContentLoaded', function() {
    ...
    [...checkboxesCategoria, ...checkboxesMarca, ...checkboxesTamano, ...checkboxesColor].forEach(checkbox => {
    ...
    function filtrarProductos() {
        ...
        tarjetasProducto.forEach(tarjeta => {
            ...
            const coincideTamano = tamanosSeleccionados.length === 0 || tamanosSeleccionados.includes(tamanoProducto);
            const coincideColor = coloresSeleccionados.length === 0 || coloresSeleccionados.includes(colorProducto);
            const coincidePrecio = precioProducto <= precioMaximo;

            // Mostrar u ocultar el producto según los filtros
            if (coincideCategoria && coincideMarca && coincideTamano && coincideColor && coincidePrecio) {
    ...
    function actualizarContadorProductos() {
        const productosVisibles = document.querySelectorAll('.product-card[style="display: block"]').length;
        const totalProductos = tarjetasProducto.length;
        document.getElementById('productos-mostrando').textContent = 
            `Mostrando ${productosVisibles} de ${totalProductos} productos`;
    }
