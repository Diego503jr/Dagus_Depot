...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function filtrarProductos() {
      ...
      productos.forEach(producto => {
        const precio = parseInt(producto.getAttribute('data-precio').replace(/,/g, ''));
  ...
  function showPage(page) {
    ...
    productosFiltrados.slice(start, end).forEach(p => p.style.display = 'block');
  ...
  function actualizarContadorProductos(paginaActual, productosPorPagina) {
    ...
    if (total === 0) {
        ...
        Swal.fire({ //ALERTA DE SWEET ALRT
...
document.querySelector(".checkout-form").addEventListener("submit", function (e) {
    ...
    // Redirigir o limpiar campos aquí si lo deseas.
  });
  ...
  document.getElementById('confirmarPedido').addEventListener('click', function (e) {
    e.preventDefault(); 
